...
const replaceZeros = (string) => {
  ...
  let value = string.replace(str1, str2);
...
const validateWord = (word) => {
  // Solution code here...
  let wordStr = /^[a-zA-Z]{5,10}$/;
  if (wordStr.test(word)) {
    return true;
  } else {
    return false;
  }
...
const hasNumber = (string) => {
  // Solution code here...
  let newString = /[a-zA-Z][0-9]/;
  if (newString.test(string)) {
    return true;
  } else {
    return false;
  }
...
const validateEmail = (email) => {
  // Solution code here...
  let newEmile = /^\w+(\.\w+)?@\w+\.(net|org|com)$/;

  if (newEmile.test(email)) {
    return true;
  } else {
    return false;
  }
