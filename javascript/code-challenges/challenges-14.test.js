...
const screenForNames = (arr) => {
  // Solution code here...
  let random = /^(Mr.|Mrs.|Ms.|Dr.)[ ][a-zA-Z]+/;
...
let biggerThanLuke = (arr) => {
  // Solution code here...
  return arr.filter(objItem => parseInt(objItem.mass) > 77).map(objItem => objItem.name).join(" - ");
};
...
const sortBy = (property, arr) => {
  arr.sort((min, max) => {
    if (min[property] > max[property]) {
      return true;
    }
    else if (max[property] > min[property]) {
      return -1;
    }
    else {
      return false;
